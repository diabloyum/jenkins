...
import { finishOptions, parseStatus, startOptions } from '../../commom/options';
...
export class OrderComponent implements OnInit {
    ...
    initCols() {
        this.cols = [
            {field: 'orderNumber', header: '订单编号'},
            {field: 'productName', header: '商品名'},
            {field: 'orderPrice', header: '价格'},
            {field: 'orderStart', header: '订单状态'},
            {field: 'orderComment', header: '评价'},
            {field: 'orderTime', header: '下单时间'}
        ];
    }
    ...
    search() {
        this.service.get('/order/').subscribe(
            (res: Array<OrderModel>) => {
                this.datas = [];
                for (const i of res) {
                    i.orderStart = parseStatus(i.orderStart, startOptions);
                    this.datas.push(i);
                }
            },
            error => {
    ...
    // }
    delete() {
        this.service.delete('order', this.selectedData.orderId).subscribe(
            res => {
                this.showSuccess();
                this.search();
            },
